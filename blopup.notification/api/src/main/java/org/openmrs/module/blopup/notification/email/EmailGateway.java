package org.openmrs.module.blopup.notification.email;
...
public class EmailGateway extends MessagingGateway {
	...
	public void sendMessage(Message message) throws Exception {
		...
		mimeMessage.setSubject(Context.getAdministrationService().getGlobalProperty(MessagingConstants.GP_EMAIL_SUBJECT));
		...
		try {
			Transport t = outSession.getTransport(Context.getAdministrationService().getGlobalProperty(
			    MessagingConstants.GP_EMAIL_OUT_PROTOCOL));
			
			t.connect(
			    Context.getAdministrationService().getGlobalProperty(MessagingConstants.GP_EMAIL_OUT_HOST),
			    Integer.parseInt(Context.getAdministrationService().getGlobalProperty(MessagingConstants.GP_EMAIL_OUT_PORT)),
			    Context.getAdministrationService().getGlobalProperty(MessagingConstants.GP_EMAIL_OUT_USERNAME),
			    encryptionService.decrypt(Context.getAdministrationService().getGlobalProperty(
	...
	public void receiveMessages() {
		...
		try {
			...
			EncryptionService encryptionService = (EncryptionService) Context.getService(EncryptionService.class);
			store.connect(Context.getAdministrationService().getGlobalProperty(MessagingConstants.GP_EMAIL_IN_HOST), Context
			        .getAdministrationService().getGlobalProperty(MessagingConstants.GP_EMAIL_IN_USERNAME),
			    encryptionService.decrypt(Context.getAdministrationService().getGlobalProperty(
	...
	public void startup() {
		...
		if (Boolean.parseBoolean(adminService.getGlobalProperty(MessagingConstants.GP_EMAIL_OUT_USEDEFAULT))) {
			// use OpenMRS default
			...
			outProps.setProperty("mail." + outProtocol + ".host", adminService.getGlobalProperty("mail.smtp_host"));
			outProps.setProperty("mail." + outProtocol + ".port", adminService.getGlobalProperty("mail.smtp_port"));
			outProps.setProperty("mail." + outProtocol + ".from", adminService.getGlobalProperty("mail.from"));
			outProps.setProperty("mail." + outProtocol + ".auth", adminService.getGlobalProperty("mail.smtp_auth"));
			outProps.setProperty("mail." + outProtocol + ".starttls.enable", adminService.getGlobalProperty("mail.use_tls"));
		} else {
			// use ours
			outProps.setProperty("mail.transport.protocol", outProtocol);
