package org.openmrs.module.blopup.notification.api.dao.hibernate;
...
import org.openmrs.module.blopup.notification.db.MessageDAO;
...
public class HibernateMessageDAO implements MessageDAO {
	...
	public Message getMessage(Integer messageId) {
		return (Message) sessionFactory.getCurrentSession().createCriteria(Message.class)
		        .add(Restrictions.eq("messageId", messageId)).list();
	}
	...
	public List<Message> getOutboxMessagesByProtocol(Protocol p) {
		Criteria c = sessionFactory.getCurrentSession().createCriteria(Message.class);
		c.add(Restrictions.or(Restrictions.eq("status", MessageStatus.OUTBOX.getNumber()),
		    Restrictions.eq("status", MessageStatus.RETRYING.getNumber())));
