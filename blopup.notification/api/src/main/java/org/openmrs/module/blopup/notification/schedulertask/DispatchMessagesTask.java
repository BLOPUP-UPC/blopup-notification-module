package org.openmrs.module.blopup.notification.schedulertask;
...
/**
 * A scheduler task that polls the database every 2 seconds looking for messages with a status of
 * 'outbox' Once such messages are found, they are dispatched to the proper gateway for sending.
 */
public class DispatchMessagesTask extends AbstractTask {
	...
	/**
	 * The maximum number of times that this system will try to send a message before marking it as
	 * failed
	 */
	...
	/**
	 * This method does the dispatching of outgoing messages. It pulls any messages from the
	 * database that have a status of 'outbox' and then dispatches them to the proper gateway.
	 */
	public void execute() {
		Context.openSession();
	...
	private Integer getMaxRetryAttempts() {
		if (maxRetryAttempts == null)
			try {
				maxRetryAttempts = Integer.parseInt(Context.getAdministrationService().getGlobalProperty(
