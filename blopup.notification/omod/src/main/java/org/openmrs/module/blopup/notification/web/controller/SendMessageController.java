...
import org.openmrs.module.blopup.notification.domain.gateway.exception.AddressFormattingException;
...
public class SendMessageController {
	...
	/**
	 * Sends a message (content) to the address specified in toAddress. If neither the toAddress nor
	 * the fromAddress are already in the OpenMRS system with an attached protocol, then a protocol
	 * must be supplied. All parameters besides content, and toAddress, are optional. It is possible
	 * to have a message that has a sender that does not own the 'fromAddress' - e.g. The user is
	 * sending from a default OpenMRS address like a mobile phone hooked up to the OpenMRS server.
	 * In this case the 'sender' and 'fromAddress' would not match up.
	 * 
	 * @param content The content of the message
	 * @param toAddressString The address to send the message to
	 * @param fromAddressString The address that the message originates from
	 * @param sender The sender of the message
	 * @param recipient The recipient of the message
	 * @param protocolId The protocol of the message
	 * @param returnUrl
	 * @param request
	 * @return
	 */
	@RequestMapping(value = "/module/blopup.notification/admin/sendMessage", method = RequestMethod.POST)
	protected String sendMessage(@RequestParam("content") String content, @RequestParam("toAddress") String toAddressString,
		...
		if (returnUrl == null || returnUrl.equals("")) {
			returnUrl = "/module/blopup.notification/admin/sendMessage.form";
		}
		...
		if (toAddress != null)
			protocol = messagingService.getProtocolById(toAddress.getProtocolId());
		if (fromAddress != null)
			protocol = messagingService.getProtocolById(fromAddress.getProtocolId());
		//if pulling the protocol from the addresses didn't work,
		// then the protocol should have been passed in as a string
		if (protocol == null)
			protocol = messagingService.getProtocolById(protocolId);
		...
		if (toAddress == null) {
			try {
				...
				httpSession.setAttribute(WebConstants.OPENMRS_ERROR_ATTR, "To-address \"" + toAddressString
				        + "\" was badly formatted.");
				return returnUrl;
			}
		}
		...
		if (fromAddress == null && fromAddressString != null && !fromAddressString.equals("")) {
			try {
				...
				httpSession.setAttribute(WebConstants.OPENMRS_ERROR_ATTR, "From-address \"" + toAddressString
				        + "\" was badly formatted.");
				return returnUrl;
			}
		}
		...
		if (!messagingService.canSendToProtocol(protocol)) {
			httpSession.setAttribute(WebConstants.OPENMRS_ERROR_ATTR,
			    "There is not currently a gateway running that can send that type of message.");
			return returnUrl;
		} else {
	...
	public void populateModel(HttpServletRequest request) {
		...
		List<MessagingAddress> fromAddresses = Context.getService(MessagingAddressService.class)
